...
int create_dummy_main(char **where, struct _test *func/*char *f_name, char **args, size_t args_sz, int flags*/)
{
	int i = 0, f = 42; // f = buf write-to offset
	char *buf;
	char *buff[50];
	int cc = 1, ac = 0;
	buf = (char *) malloc((4096 + strlen(func->func->name) + func->func->arg_len * 16 + func->test_value_sz * 16) * sizeof(char)); // MAGIC NUMBERS!!!
	if (buf == NULL) 
		return 1;
	...
	if (func->flags == FLG_TEST_RET) {
		...
		while (cc <= func->func->arg_len) {
			fprintf(stderr, "%d ( %d )\n", (func->flags & FLG_TEST_ARG(cc)) % 2, cc);
			if (func->flags & FLG_TEST_ARG(cc)) {
				fprintf(stderr, "%s test_arg_%d; %dth arg\n", *(func->func->arg_type + ac), ac, cc);
				sprintf(buf + strlen(buf), "%s test_arg_%d;\n", *(func->func->arg_type + ac), ac);
				ac++;
			}
			cc++;
		}
	...
	cc = 0;
	sprintf(buf + strlen(buf), "%s func_ret = %s (", func->func->ret_t, func->func->name);
	while (i < func->func->arg_len) {
		if (func->flags & FLG_TEST_ARG(i + 1)) {
			sprintf(buf + strlen(buf), "&test_arg_%d, ", cc);
			cc++;
		} else {
	...
	if (func->flags & FLG_TEST_RET) {
		sprintf(buf + strlen(buf), "if (func_ret == %s) {\n\t\tprintf(\"Return value is correct.\\n\");\n\t} else {\n\t\tprintf(\"Return value is not correct.\\n\");\n\t}\n", *func->test_value);
	}
	if (ac) {
		i = 0;
		while (i < func->test_value_sz - 1) {
			sprintf(buf + strlen(buf), "\tif (test_arg_%d == %s) {\n\t\tprintf(\"Argument #%%d is correct.\\n\", %d);\n\t} else {\n\t\tprintf(\"Argument #%%d is not correct.\\n\", %d);\n\t}\n", i, *(func->test_value + i + 1), i, i);
			i++;
		}
	}
	sprintf(buf + strlen(buf), "return 0;\n}");
	*where = buf;
return 0;
}
...
struct _test *parse_command_string(int fd)
{
	int i = 0, s = 0, d = 0, c = 0, inc_size = 0;
	...
	lseek(fd, *special_offsets, SEEK_SET);
...
int main(int argc, const char *argv[]) {
	...
	struct _test *f = parse_command_string(fd);
	char *mainz;
	create_dummy_main(&mainz, f);
	lseek(fd, 0, SEEK_SET);
