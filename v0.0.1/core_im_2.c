...
void destroy_test(struct _test *str)
{
	...
	while (tc < str->test_cnt) {
		while (i < *(str->test_value_sz + tc)) {
			free(*(*(str->test_value + tc)) + i);
			i++;
		}
		free(*(str->test_value + tc));
		tc++;
	}
	free(str->test_value_sz);
	free(str->test_value);
	free(str->flags);
	free(str);
}
...
int create_dummy_main(char **where, struct _test **test, size_t test_count/*char *f_name, char **args, size_t args_sz, int flags*/)
{
	...
	int i = 0, f = 42; // f = buf write-to offset
	char *buf;
	char *buff[50];
	int cc = 1, *ac, tc = 0, ttc = 0;
	int alloc_size = (1024 + (strlen(func->func->name) + 32) * test_count + func->func->arg_len * 32 * test_count + func->test_cnt * 64 * test_count); // Magic numbers
	buf = (char *) malloc(alloc_size * sizeof(char));
	if (buf == NULL) 
		return 1;
	ac = (int *) malloc(test_count * sizeof(int *));
	bzero(ac, test_count * sizeof(int *));
	sprintf(buf ,"#include<cassert>\nint main(int argc, const char *argv[]) {\n\tprintf(\"\x1B[31m\");\n\t");
	while (tc < test_count) {
		func = *(test + tc);
		while (ttc < func->test_cnt) {
			while (cc <= func->func->arg_len) {
				if (*(func->flags + ttc) & FLG_TEST_ARG(cc)) {
					sprintf(buf + strlen(buf), "%s test_arg_%d_%d_%d;\n", *(func->func->arg_type + ac[tc]), ttc, tc, ac[tc]);
					++ac[tc];
				}
				cc++;
			}
	...
	while (tc < test_count) {
		func = *(test + tc);
		while (ttc < func->test_cnt) {
			sprintf(buf + strlen(buf), "\t%s func_ret_%d_%d = %s (%c", func->func->ret_t, ttc, tc, func->func->name, 0);
			i = 0;
			while (i < func->func->arg_len) {
				if (*(func->flags + ttc) & FLG_TEST_ARG(i + 1)) {
					sprintf(buf + strlen(buf), "&test_arg_%d_%d_%d, %c", ttc, tc, cc, 0);
					cc++;
				} else {
	...
	while (tc < test_count) {
		func = *(test + tc);
		while (ttc < func->test_cnt) {
			if (*(func->flags + ttc) & FLG_TEST_RET) {
				sprintf(buf + strlen(buf), "\tif (func_ret_%d_%d == %s) {\n\t\tprintf(\"Return value of function `%s` is correct.\\n\");\n\t} else {\n\t\tprintf(\"Return value is not correct.\\n\");\n\t}\n%c", ttc, tc, **(func->test_value + ttc), func->func->name ,0);
			}
			if (ac[tc]) {
				i = 0;
				while (i < *(func->test_value_sz + ttc) - 1) {
					sprintf(buf + strlen(buf), "\tif (test_arg_%d_%d_%d == %s) {\n\t\tprintf(\"Argument #%%d of function `%s` is correct.\\n\", %d);\n\t} else {\n\t\tprintf(\"Argument #%%d of function `%s` is not correct.\\n\", %d);\n\t}\n%c", ttc, tc, i, *(*(func->test_value + ttc) + i + 1), func->func->name, i, func->func->name, i, 0);
					i++;
				}
			}
			ttc++;
	...
	fprintf(stderr, "Main function looks like this:\n%s\n", buf);
	*where = buf;
return 0;
}
...
{
	int i = 0, d = 0, s = 0, argc = 0, last_size = 1, tc = 0, fc = 0;
	...
	++ret->test_cnt;
	ret->test_value_sz = (int *) malloc(ret->test_cnt * sizeof(int));
	ret->flags = (unsigned long long int *) malloc(ret->test_cnt * sizeof(unsigned long long int));
	ret->test_value = (char ***) malloc(ret->test_cnt * sizeof(char **));
	while (*string) {
		argc = 0;
		if (*string == SP_TEST_CHAR) {
			*(ret->flags + tc) |= FLG_TEST_RET;
			d = 1;
			string++;
			while (*(string) != '(') {
				(d-1)[valbuf] = *string;
			...
			*(ret->test_value_sz + tc) = 1;
			*(ret->test_value + tc) = (char **) malloc(1 * sizeof(char *));
			**(ret->test_value + tc) = (char *) malloc(*(ret->test_value_sz) * sizeof(char));
			if (*(ret->test_value) == NULL || **(ret->test_value + tc) == NULL) {
				fprintf(stderr, "ERROR: Cannot allocate memory\n");
				exit(1);
			}
			sprintf(*(*(ret->test_value + tc)), "%s%c", valbuf, 0);
		} else if (*string == '|') {
			string++;
		} else if (*string == ')') {
			tc++;
		...
		d = 0;
		string -= fc;
		fprintf(stderr, "++ strlen(string) = %d\n", strlen(string));
		while (*(string + d)) {
			fprintf(stderr, "FUCK %c\n", *(string +d));
			if (*(string + d) == ',') {
				argc++;
			} else if (*(string + d) == ')') {
				break;
				...
				d++;
				*(ret->flags + tc) |= FLG_TEST_ARG(argc + 1);
				++ *(ret->test_value_sz + tc);
				s = 0;
				...
				*(ret->test_value + tc) = (char **) realloc(*(ret->test_value + tc), *(ret->test_value_sz + tc) * sizeof(char *));
				*(*(ret->test_value + tc) + *(ret->test_value_sz + tc) - 1) = (char *) malloc((strlen(valbuf) + 1) * sizeof(char));
				if (*(ret->test_value + tc) == NULL || *(*(ret->test_value + tc)) == NULL) {
					fprintf(stderr, "ERROR: Cannot allocate memory\n");
					exit(1);
				}
				fprintf(stderr, "Argument should be: %s\n", valbuf);
				sprintf(*(*(ret->test_value + tc) + *(ret->test_value_sz + tc) - 1), "%s%c", valbuf, 0);
				bzero(valbuf, 64);
				continue;
			}
			string += d + 1;
	...
	*fun->args = (char *) malloc(last_size * sizeof(char));
	...
	d = 0;
	while (*(string + d)) {
		fprintf(stderr, "Looping though: %c(%d)\n", *(string + d), *(string +d));
	...
	fprintf(stderr, "FUCK: arg_len = %d; first arg = %s\n", fun->arg_len, *fun->args);
	ret->func = fun;
return ret;
}
...
struct _test **parse_command_string(int fd, size_t *ret_count) // _IM_
{
	...
	*ret_count = ret_tot;
	...
	while (ret_c < ret_tot) {
		...
		*(buf + i) = '\0';
		fprintf(stderr, "Passing %s to the motherfucking function!\n", buf);
		...
		*(buff + s) = '\0';
		...
		fun->ret_t = i_hate_this; // I really do.
		while (*(buff + c) == ' ' || *(buff + c) == '\n' || *(buff + c) == '\t') 
			c++;
		while (*(buff + c) != '(' && *(buff + c) != ' ') {
			*(name + d) = *(buff + c);
		...
		*(name + d) = '\0';
		lseek(fd, 0, SEEK_SET);
		fprintf(stderr, "some_arg_len = %d\n", ret->func->arg_len);
		fun1 = parse_function(fd, name, inc, &inc_size);
		while (inc_size)
			destroy_incl(*(inc + inc_size--));
